...
- **A downloadable disk image.** `scripts/build-dmg.sh` packages the app with an
  Applications symlink and the app's own volume icon, and refuses to pretend the
  result is distributable: it prints exactly what Gatekeeper will do with an ad
  hoc or self-signed build.
- **Tag-driven releases.** Pushing `v0.2.0` stamps the version from the tag with
  `scripts/set-version.sh`, runs the tests, builds the disk image and publishes a
  GitHub release whose notes say plainly whether the build is notarised. Signing
  and notarisation switch on automatically once the repository secrets exist, and
  are documented in `docs/releasing.md`.
- **README images**, generated by `DocumentationScreenshotTests` rather than
  drawn by hand: it renders Snaplet's real selection overlay, editor canvas and
  presentation renderer over a synthetic desktop of its own design, so the
  pictures contain no personal content and anyone can reproduce them.
...
- The settings window is wider by default, leaving the tab bar room in longer
  languages.
- Tests now run with `-AppleLanguages (en)` from the shared scheme. Results used
  to depend on the machine's language — a zoom test compared against "Fit" and
  failed on a Turkish system.
