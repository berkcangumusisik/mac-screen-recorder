...
Press the shortcut and drag. The overlay dims the screen, reports the exact
pixel size, and the loupe magnifies whatever is under the pointer — press
<kbd>C</kbd> and its colour is on your clipboard.

![Selecting an area, with the pixel loupe and colour readout](docs/images/selection-overlay.png)

Mark it up without touching the original: arrows, numbered steps, shapes, and an
opaque redaction over the part that should not leave your machine.

![The editor with an arrow, a numbered step and a redaction](docs/images/editor.png)

Then drop it on a background with padding, a neutral window frame and a caption,
and export at the size you need.

![The same capture with the Studio preset applied](docs/images/styling.png)

> These images are generated by `DocumentationScreenshotTests`, which renders
> Snaplet's real views and renderers over a synthetic desktop of its own design.
> Regenerate them with the command in
...
- The images above are rendered from Snaplet's own views over a synthetic
  desktop. A recorded demo of the app running is still to be made; the scenarios
  are written down in [docs/demo-scenarios.md](docs/demo-scenarios.md).
